...
import { User, Mail, Lock, Loader2, Moon, Sun } from 'lucide-react';
  ...
  return (
    <div className="w-full relative">
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        <div>
          <label className="block text-sm font-medium text-brand-text-muted mb-2 px-1">Full Name</label>
          <div className="relative group">
            <User className="w-5 h-5 text-brand-text-muted absolute left-4 top-1/2 -translate-y-1/2 group-focus-within:text-brand-primary transition-colors" />
            <input 
              type="text" 
              required
              autoComplete="name"
      ...
      <p className="mt-8 text-center text-sm text-brand-text-muted">
        Already have an account? <Link to="/login" className="text-brand-primary hover:text-brand-primary-hover hover:underline font-semibold ml-1">Sign in</Link>
      </p>
    </div>
  );
}
